...
# Mac M4 自动监控重启脚本
# 基于最新 always_run_rl_swarm.sh 的配置，自动化交互参数
# 监控进程状态，自动重启
...
# 默认参数配置（基于最新 always_run_rl_swarm.sh）
...
cleanup() {
    ...
    pkill -f "always_run_rl_swarm.sh" 2>/dev/null || true
...
start_training() {
    ...
    # 设置 always_run_rl_swarm.sh 需要的环境变量
...
# 检查是否在正确的目录
if [ ! -f "always_run_rl_swarm.sh" ]; then
